...
courses= pd.read_csv(r'C:\Users\Tifa\Desktop\python\Coursera_courses_catalog.csv')
print(courses.head())
print(courses.info())
...
for i in courses['course_level']:
    list6.append(i)
ax = pd.Series(list6).value_counts()[:10].sort_values(ascending=True).plot.barh(width=0.9,color=sns.color_palette('hls',10))
for i, v in enumerate(pd.Series(list6).value_counts()[:10].sort_values(ascending=True).values): 
    ax.text(.8, i, v,fontsize=12,color='white',weight='bold')
